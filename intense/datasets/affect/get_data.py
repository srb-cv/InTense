...
def scale_labels(labels, scaler=None):
    sq_labels = np.squeeze(labels)
    sentiment_labels = sq_labels[:,0]
    sentiment_labels = sentiment_labels[:,None]
    if scaler is None:
        scaler = MinMaxScaler(feature_range=(-1,1))
        scaler.fit(sentiment_labels)
    normalized_labels = scaler.transform(sentiment_labels)
    normalized_labels = np.squeeze(normalized_labels)
    sq_labels[:, 0] = normalized_labels
    return sq_labels[:, None, :], scaler
...
class Affectdataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        label = torch.tensor(
            np.array(_get_class(tmp_label))
            ).long() if self.task == "classification" else torch.tensor(tmp_label).float()
...
def get_dataloader(
        ...
        raw_path='/home/van/backup/pack/mosi/mosi.hdf5', z_norm=False, save_path='./', label_norm=False) -> DataLoader:
    ...
    def _save_hparams() -> None:
        ...
        with open(os.path.join(save_path, "data_hparams.yaml"), 'w') as file:
    ...
    if label_norm:
        processed_dataset['train']['labels'], train_scaler = scale_labels(
            processed_dataset['train']['labels'])
        processed_dataset['valid']['labels'], _ = scale_labels(
            processed_dataset['valid']['labels'], scaler=train_scaler)

    train = DataLoader(Affectdataset(processed_dataset['train'],
                                     flatten_time_series,
                                     task=task, max_pad=max_pad,
                       ...
                       batch_size=batch_size,
                       collate_fn=process)
    valid = DataLoader(Affectdataset(processed_dataset['valid'],
                                     flatten_time_series,
                                     task=task,
                                     max_pad=max_pad,
                                     max_pad_num=max_seq_len,
                       ...
                       batch_size=batch_size,
                       collate_fn=process)
    test = DataLoader(Affectdataset(processed_dataset['test'],
                                    flatten_time_series,
                                    task=task, max_pad=max_pad,
                                    max_pad_num=max_seq_len,
                                    data_type=data_type,
...
def _process_1(inputs: list):
    ...
    for i in range(len(inputs[0]) - 2):
        ...
        processed_input_lengths.append(torch.as_tensor([v.size(0)
                                                        for v in feature]))
    ...
    for sample in inputs:
        ...
        if sample[-1].shape[1] > 1:
            labels.append(sample[-1].reshape(sample[-1].shape[1],
                                             sample[-1].shape[0])[0])
        else:
...
def _process_2(inputs: list):
    ...
    return processed_input[0], processed_input[1], \
        processed_input[2], torch.tensor(labels).view(len(inputs), 1)


if __name__ == '__main__':
    traindata, validdata, test_robust = get_dataloader(
        './humor.pkl', robust_test=False,
        max_pad=True, task='classification',
        data_type='humor', max_seq_len=40)

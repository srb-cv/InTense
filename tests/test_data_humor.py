def test_humor_data():
    import pickle
    with open("/work/ML/varshneya/humor.pkl", 'rb') as f:
        data = pickle.load(f)
    ...
    assert test_data['audio'].shape == (1058, 20, 81)
    assert test_data['vision'].shape == (1058, 20, 371)
    assert test_data['text'].shape == (1058, 20, 300)


def test_humor_dataloader_v1():
    '''
...
def test_humor_dataloader_mosei_v2():
    '''

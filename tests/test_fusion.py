...
def test_mkl_regularizer_one_output():
    ...
    import torch
    fusion = MKLFusion(in_features={'1': 1, '2': 2, '3': 2},
                       out_features=1)
...
def test_mkl_scores_one_output():
    ...
    custom_weights = torch.Tensor([1, 2, 3, 1, 2]).unsqueeze(0)
    fusion.fusion_layer.weight.copy_(custom_weights)
    modality_norms = [torch.linalg.matrix_norm(
                        torch.Tensor([1]).unsqueeze(0)
                        ),
                      torch.linalg.matrix_norm(
                        torch.Tensor([2, 3]).unsqueeze(0)
                        ),
                      torch.linalg.matrix_norm(
    ...
    scores = scores.numpy()
    scores = dict(zip(['1', '2', '3'], scores))
    print(scores)
    print(fusion.scores())
    assert scores == fusion.scores()
...
def test_mkl_scores_multiple_outputs():
    ...
    modality_norms = [torch.linalg.matrix_norm(
        ...
        torch.linalg.matrix_norm(
            torch.Tensor([2, 3, 1, 1]).unsqueeze(0)
        ),
        torch.linalg.matrix_norm(
            torch.Tensor([1, 2, 4, 1]).unsqueeze(0)
        )
        ]
    sum_norms = torch.sum(torch.Tensor(modality_norms))
    scores = torch.Tensor([norm_ / sum_norms for norm_ in modality_norms])
